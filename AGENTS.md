...
### Branches
- **Create a feature branch for every change.** Use `feat/*`, `fix/*`, `refactor/*`, `docs/*`, `ci/*` naming.
- **Never commit directly to `main`.** All changes go through a PR.
- **Open a PR when work is done.** Follow the `stank-pr-workflow` skill.
- **PRs require user approval before merge.** Do not merge PRs yourself.

### Commits
- **Use conventional commit format:** `type(scope): description`
- **No `Co-Authored-By` trailer.** Never add AI co-author trailers.
- **Do NOT bump the version.** Version bump happens at deploy time only (`stank-deploy`).
- Commit and push to your feature branch, not main.
...
| `POST /api/mock/media` | Inject a fake media item with optional generated history |
| `POST /api/mock/clear-media` | Delete all media for the active guild |
...
- **Embed rendering:** [src/stankbot/services/board_renderer.py](src/stankbot/services/board_renderer.py) + [template_engine.py](src/stankbot/services/template_engine.py) + [template_store.py](src/stankbot/services/template_store.py) + [default_templates.py](src/stankbot/services/default_templates.py) (source of `ALL_DEFAULTS`) + [embed_builders.py](src/stankbot/services/embed_builders.py).
- **Cogs (Discord surface):** [src/stankbot/cogs/](src/stankbot/cogs/).
- **Scheduling:** [src/stankbot/scheduling/](src/stankbot/scheduling/) — APScheduler wrappers. `session_scheduler.py` (per-guild reset rollover) and `media_metrics_scheduler.py` (clock-aligned media metric refresh). Both rebuild jobs from guild settings on each boot — no schedule state is persisted.
- **Media analytics:** [src/stankbot/services/media_service.py](src/stankbot/services/media_service.py) (guild-scoped CRUD + refresh + history; items are identified per-type by a `name` slug) + [media_providers/](src/stankbot/services/media_providers/) (`base.py`, `registry.py`, `youtube.py`, `spotify.py`, `mock_providers.py` for `dev-mock`; admins enable/disable providers per-guild via the `MEDIA_PROVIDERS_ENABLED` setting — non-admins are filtered, admins always see everything) + [cogs/media_commands.py](src/stankbot/cogs/media_commands.py) (`/stats youtube info|chart`, `/stats spotify info|chart`) + [services/chart_renderer.py](src/stankbot/services/chart_renderer.py) (Pillow PNG renderer for the chart-image endpoint, `mode=total|delta`) + [scheduling/media_metrics_scheduler.py](src/stankbot/scheduling/media_metrics_scheduler.py) (clock-aligned per-guild interval refresh, default 60 min, floor 5 min; also runs `cleanup_chart_cache` every 10 min) + [web/routes/media_api.py](src/stankbot/web/routes/media_api.py) + [media_admin.py](src/stankbot/web/routes/media_admin.py) + [mock_media.py](src/stankbot/web/routes/mock_media.py) + frontend [routes/media/](src/stankbot/web/frontend/src/routes/media/) and [routes/admin/media/](src/stankbot/web/frontend/src/routes/admin/media/).
