...
describe('api.ts', () => {
	it('apiFetch returns parsed JSON on 200', async () => {
		...
		const out = await apiFetch<{ ok: boolean; n: number }>('/v2/api/x', {
	...
	it('apiFetch retries 5xx then succeeds', async () => {
		...
		const out = await apiFetch<{ ok: boolean }>('/v2/api/x', {
			fetch: fn as unknown as typeof fetch
		});
	...
	it('apiFetch throws FetchError on 4xx immediately', async () => {
		...
		await expect(
			apiFetch('/v2/api/x', { fetch: fn as unknown as typeof fetch })
