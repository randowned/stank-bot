<script lang="ts">
	...
	async function load() {
		try {
			doc = await apiFetch<TemplatesDoc>('/v2/api/admin/templates');
	...
	async function runPreview() {
		...
		try {
			const res = await apiPost<{ rendered: Record<string, unknown> }>(
				`/v2/api/admin/templates/${activeKey}/preview`,
	...
	async function save() {
		...
		try {
			...
			await apiPost(`/v2/api/admin/templates/${activeKey}`, { data: parsed });
			saveMsg = 'Saved.';
			await load();
		} catch (err) {
