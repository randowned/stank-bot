<script lang="ts">
	...
	async function load() {
		try {
			doc = await apiFetch<RolesDoc>('/v2/api/admin/roles');
		} catch (err) {
	...
	async function addRole() {
		...
		try {
			await apiPost('/v2/api/admin/roles/add', { role_id: Number(newRole) });
			newRole = '';
			await load();
		} catch (err) {
	...
	async function removeRole(role: string) {
		try {
			await apiPost('/v2/api/admin/roles/remove', { role_id: Number(role) });
			await load();
		} catch (err) {
	...
	async function addUser() {
		...
		try {
			await apiPost('/v2/api/admin/roles/users/add', { user_id: Number(newUser) });
			newUser = '';
			await load();
		} catch (err) {
			error = err instanceof FetchError ? err.message : 'Add failed';
	...
	async function removeUser(uid: string) {
		try {
			await apiPost('/v2/api/admin/roles/users/remove', { user_id: Number(uid) });
			await load();
		} catch (err) {
			error = err instanceof FetchError ? err.message : 'Remove failed';
		}
