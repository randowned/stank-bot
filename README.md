...
- **Media analytics** for YouTube and Spotify. Track videos/albums via the admin panel, view metric history and comparison charts on the dashboard, browse channel/artist profiles with cover photos and owner-level charts, and query stats in Discord with `/stats youtube info <name>` / `/stats spotify info <name>` / `/stats youtube chart <name> [type] [range]` / `/stats youtube channel` / `/stats spotify artist`. Per-provider embed templates, scheduled metric snapshots, configurable refresh intervals, public chart image endpoint, and configurable milestone announcements (sparse K-scale: 1K, 5K, 10K, 25K, 50K, 100K, 250K, 500K, 750K; then every 1M views/plays through 50M then 75M / 100M / 150M / ... / 1B) — with celebratory embeds pushed to announcement channels. Channel/artist owner milestones (subscribers + total views for YouTube; followers + total plays for Spotify) are announced with the same pipeline.
...
| `/stats youtube channel` | Channel overview embed: subscriber count, video count, aggregate total views/likes/comments from tracked videos. |
| `/stats spotify artist` | Artist overview embed: follower count, popularity score, aggregate total plays from tracked items. |
...
- `/media/{id}` — single media item detail: provider-aware metric tiles (1 tile for Spotify, 3 for YouTube), time-scale history chart with range options (1h / 6h / 12h / 24h / 48h / 7d / 30d / 90d / 1y, default 48h), defaults to Change (delta) mode, view mode toggle (Change / Cumulative) with alignment-filtered rendering (5 Min / 15 Min / 30 Min / Hourly / Daily / Weekly / Monthly resolution), data-start indicator when data is shorter than selected range, multi-series comparison overlay (type-filtered), staleness pill in header, "Open on YouTube / Spotify" link, sparse-data hint when only one snapshot exists, gap-free line rendering. All chart controls (metric, range, resolution, mode, compare IDs) are reflected in the URL query string and update without page reload via `history.replaceState()` — share or reload preserves the exact chart view. StatTiles and media cards flash green on live metric updates pushed via WebSocket. Milestone achievements (1M+ views/plays) trigger a dismissible slide-up overlay in the dashboard. The inline channel/artist card links to the full profile page.
- `/media/profiles` — profiles (Artists) listing: provider type tabs (All / YouTube / Spotify), grid of profile cards each showing cover photo (channel banner / artist image), avatar, provider badge, top 5 owner metrics (subscribers / total views / total likes / total comments / video count for YouTube; followers / popularity / total plays for Spotify) with computed aggregates from media items when the provider API doesn't supply them, tracked item count, and freshness indicator. Clicking a card navigates to the profile detail page.
- `/media/profile/{id}` — channel/artist profile detail: cover photo hero with gradient overlay and avatar, owner metric StatTiles in panel containers with live WebSocket flash updates, interactive time-series chart (same controls as media detail: metric, range, resolution, delta/cumulative mode) for owner-level snapshots (subscribers, total views, etc.), and a 2-column grid of tracked media items using the shared MediaItemCard component (provider-colored border, thumbnail, title, published date, metric chips).
...
- Admin `/admin/media/{id}/edit` — edit page: editable name field with save, metadata summary, and tabbed snapshot tables (Media Snapshots + Channel/Artist Snapshots) showing the last 20 pivoted snapshots for each.
- Header: single row, `Live updates` badge for non-admin users (gray when logged out, green/muted/red when connected) or `N online` badge for admin users (clickable popover with avatars + admin tag + session durations), user menu with Navigate (Dashboard / Sessions / Media / Artists) + My Profile + collapsible Switch Guild showing the active guild's icon + name.
