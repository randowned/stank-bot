...
- `/media/profiles` — profiles listing: provider type tabs (All / YouTube / Spotify), grid of profile cards each showing cover photo (channel banner / artist image), avatar, provider badge, top 3 owner metrics (subscribers / total views / video count for YouTube; followers / popularity / total plays for Spotify), tracked item count, and freshness indicator. Clicking a card navigates to the profile detail page.
- `/media/profile/{id}` — channel/artist profile detail: cover photo hero with gradient overlay and avatar, owner metric StatTiles with live WebSocket flash updates, interactive time-series chart (same controls as media detail: metric, range, resolution, delta/cumulative mode) for owner-level snapshots (subscribers, total views, etc.), and a grid of all tracked media items from that channel/artist with thumbnail, title, provider, and current metrics.
