"""JSON admin API for the SvelteKit v2 dashboard.

Mirrors the Jinja admin routes in ``stankbot.web.routes.admin`` but returns
JSON (never HTML or redirects) so the SvelteKit frontend can consume them
as a conventional REST API. Every route requires ``require_guild_admin``,
writes the same audit-log entries as the old site, and calls the same
services — no direct DB access except where ``admin.py`` already does it.

This module is mounted independently of the old admin router; both are
live during the retirement transition so any divergence is caught early.
"""
...
async def save_template(
    ...
    # Validate every string-valued field catches snake_case typos in
    # template variables so bad tokens surface before save, not at render
    # time in Discord.
    try:
...
async def preview_template(
    ...
    We don't return a ``discord.Embed`` here (that's an SDK object) — instead
    we substitute ``{variables}`` manually and return the resulting dict.
    The frontend renders the card, which is fine because Discord embeds are
    mostly static data (title/desc/fields/etc).
    """
    from stankbot.services.default_templates import ALL_DEFAULTS
