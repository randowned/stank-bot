<script lang="ts">
    ...
    function metricValue(item: MediaItem, key: string): number {
        return (
            liveMetricsCache[item.id]?.[key] ?? item.metrics?.[key]?.value ?? 0
        );
    }

    const filteredItems = $derived.by(() => {
        let list = activeType
            ? items.filter((i) => i.media_type === activeType)
            : items;
        ...
        switch (sortKey) {
            ...
            case "updated_desc":
                sorted.sort(
                    (a, b) =>
                        ts(b.metrics_last_fetched_at) -
                        ts(a.metrics_last_fetched_at),
                );
                break;
            case "views_desc":
                sorted.sort(
                    (a, b) => primaryMetricValue(b) - primaryMetricValue(a),
                );
                break;
            case "title_asc":
    ...
    function metricsForCard(
        item: MediaItem,
    ): Array<{ key: string; label: string; icon: string; display: string }> {
        const provider = $providersByType[item.media_type];
        const out: Array<{
            key: string;
            label: string;
            ...
            out.push(
                {
                    ...
                    label: "Views",
                    icon: "👁️",
                    display: formatNumber(metricValue(item, "view_count")),
                },
                {
                    key: "like_count",
                    label: "Likes",
                    icon: "👍",
                    display: formatNumber(metricValue(item, "like_count")),
                },
                {
                    key: "comment_count",
                    label: "Comments",
                    icon: "💬",
                    display: formatNumber(metricValue(item, "comment_count")),
                },
            );
        }
    ...
    function compareSelected() {
        ...
        void goto(
            `${base}/media/${primary}?compare=${extras.join(",")}&metric=${metric}&days=2`,
        );
    }
...
<div class="max-w-6xl mx-auto p-4 space-y-4">
    ...
    {#if filteredItems.length === 0}
        <EmptyState
            icon="📊"
            title="No media yet"
            message="Admins can add YouTube and Spotify media from the admin panel."
        />
    {:else}
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {#each filteredItems as item (item.id)}
                {@const freshness = formatFreshness(
                    item.metrics_last_fetched_at,
                    10,
                )}
                    ...
                    class="panel overflow-hidden p-0 block hover:border-accent transition-colors relative {providerBrandBorder(
                        ...
                        ? selectedIds.includes(item.id)
                            ...
                            : typeLocked
                              ...
                              : 'opacity-50'
                        : ''} {flashItems[item.id] ? 'row-flash' : ''}"
                >
                    {#if compareMode}
                        <button
                            type="button"
                            class="absolute top-2 right-2 z-10 w-6 h-6 rounded border-2 flex items-center justify-center {selectedIds.includes(
                                item.id,
                            )
                                ? 'bg-accent border-accent text-white'
                                : 'border-muted bg-panel'} {typeLocked
                                ? 'pointer-events-none'
                                : ''}"
                            onclick={(e) => {
                                e.preventDefault();
                    ...
                    {/if}
                    <a
                        href={compareMode && !typeLocked
                            ? "#"
                            : `${base}/media/${item.id}`}
                        {...compareMode && !typeLocked
                            ? {
                    ...
                    >
                        ...
                        {:else}
                            <div
                                class="w-full h-40 bg-border flex items-center justify-center text-muted text-4xl"
                            >
                                ▶️
                            </div>
                        {/if}
                        <div class="p-3">
                            <h3
                                class="font-semibold text-text line-clamp-2 text-sm mb-1"
                            >
                                {item.title}
                            </h3>
